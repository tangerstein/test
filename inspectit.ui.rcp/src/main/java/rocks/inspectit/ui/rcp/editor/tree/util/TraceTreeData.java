 ...
 * <p>
 * Implements {@link Span} interface so that we can use in the commands as a span (delegates calls
 * to original span in the data).
 *
 * @author Ivan Senic
 *
 */
public final class TraceTreeData implements Comparable<TraceTreeData>, Span {
	 ...
	 */
	public static List<InvocationSequenceData> collectInvocations(TraceTreeData data, List<InvocationSequenceData> list) {
		list.addAll(data.getInvocations());
	 ...
	 * @return returns given list for easier connecting.
	 */
	public static List<Span> collectSpans(TraceTreeData data, List<Span> list) {
		list.add(data.getSpan());
	...
	private boolean isParentFollowsFromCaller() {
		return (null != parent) && parent.isCaller() && References.FOLLOWS_FROM.equals(parent.getReferenceType());
	}
	...
	private boolean isFollowsFromCallee() {
		return !isCaller() && References.FOLLOWS_FROM.equals(getReferenceType());
	}

	/**
	 * Returns combined tags from both spans. Client tags overwrite server ones.
	 *
	 * @return Returns combined tags from both spans.
	 */
	@Override
	...
	/**
	 * Propagation type. Always goes client first.
	 *
	 * @return Propagation type.
	 */
	@Override
	public PropagationType getPropagationType() {
		return span.getPropagationType();
	}

	/**
	 * Reference. Always goes client first.
	 *
	 * @return Reference.
	 */
	@Override
	public String getReferenceType() {
		return span.getReferenceType();
	}

	/**
	 * Method ident. Always goes server first.
	 *
	 * @return Platform ident.
	 */
	@Override
	public long getMethodIdent() {
		return span.getMethodIdent();
	}

	/**
	 * Platform ident. Always goes server first.
	 *
	 * @return Platform ident.
	 */
	@Override
	public long getPlatformIdent() {
		return span.getPlatformIdent();
	}

	/**
	 * Timestamp. Always goes server first.
	 *
	 * @return Timestamp
	 */
	@Override
	...
	public double getExclusiveDuration() {
		...
		double exclusive = getDuration() - d;
		return exclusive;
	}
